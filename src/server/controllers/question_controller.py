...
from ..dto.response_dto import ExampleQuestionDTO
...
class QuestionController:
    ...
    def create_question_response():
        ...
        try:
            ...
            is_valid, error_msg = RequestHandler.validate_required_fields(
                data, ["user_id", "question", "response"]
            )
            if not is_valid:
            ...
            if DatabaseHelper.save(new_response):
                return ResponseHandler.success(
                    "Question response created successfully",
                    {"response": new_response.to_dict()},
                    201,
                )
            else:
                return ResponseHandler.server_error(
                    "Failed to create question response"
                )
    ...
    def get_question_response(response_id: str):
        ...
        try:
            ...
            return ResponseHandler.success(
                "Question response retrieved successfully",
    ...
    def delete_question_response(response_id: str):
        ...
        try:
            ...
            if DatabaseHelper.delete(response):
                ...
                return ResponseHandler.server_error(
                    "Failed to delete question response"
                )
    ...
    def get_example_questions(self):
        ...
        try:
            ...
            # Convert to DTOs
            question_dtos = [
                ExampleQuestionDTO(**q) for q in self.assistant.get_example_questions()
            ]

            if category:
                filtered_dtos = [
                    dto
                    for dto in question_dtos
            ...
            return ResponseHandler.success(
                "Example questions retrieved successfully",
                {
                    ...
                    "categories": list(
                        set(
                            q["category"]
                            for q in self.assistant.get_example_questions()
                        )
                    ),
                },
            )
