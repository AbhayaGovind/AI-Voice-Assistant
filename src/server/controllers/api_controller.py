...
from ..models.models import QuestionResponse, User

# Add parent directory to path to import assistant modules
# sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))

try:
    from ...ai_assistant import AISingleton, get_ai_assistant, initialize_ai_assistant
...
class AssistantAPIController:
    ...
    def ask_assistant(cls):
        ...
        try:
            ...
            if ASSISTANT_AVAILABLE and AISingleton.is_initialized():
                try:
                    ...
                    if assistant.response is not None:
                        status = "answered"
                    response_text = (
                        assistant.response or "I'm not sure how to respond to that."
                    )
                except Exception as e:
            ...
            return ResponseHandler.success(
                "Question processed successfully", response_dto.to_dict()
            )
    ...
    def get_user_conversation(user_id: str):
        ...
        try:
            ...
            conversations = (
                QuestionResponse.query.filter_by(user_id=user_id)
            ...
            return ResponseHandler.success(
                "Conversation history retrieved successfully", response_dto.to_dict()
            )
    ...
    def get_assistant_status(cls):
        ...
        try:
            ...
            status_dto = AssistantStatusDTO(
                ...
                ai_provider=assistant.ai_provider.name if assistant else None,
                response_time=(
                    assistant.ai_provider.response_time if assistant else None
                ),
            )

            return ResponseHandler.success(
                "Assistant status retrieved successfully", status_dto.to_dict()
            )
    ...
    def reset_assistant(cls):
        ...
        try:
            ...
            return ResponseHandler.success(
                "Assistant conversation reset successfully", {"status": "reset"}
            )
