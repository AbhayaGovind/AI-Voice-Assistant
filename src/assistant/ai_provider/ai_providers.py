...
class AIProvider(ABC):
    ...
    def add_QandA(self, question: str, answer: str) -> None:
        q_and_a = QuestionAndAnswer(question=question, answer=answer)
    ...
    def ask_with_timeout(self, prompt: str) -> str:
        ...
        self._thread = threading.Thread(
            target=self._ask_with_stop_check, args=(prompt,)
        )
    ...
    def _generic_ask(self, prompt: str) -> str:
        ...
        try:
            ...
            self.add_QandA(self.messages[-1]["content"], self.answer)
            return self.answer
        except Exception as e:
            self.status = AiProviderStatus.ERROR
    ...
    def show_conversation_history(self) -> None:
        ...
        for i, msg in enumerate(self.messages):
            print(
                f"  {i+1}. {msg['role'].upper()}: {msg['content'][:100]}{'...' if len(msg['content']) > 100 else ''}"
            )
        print()
    ...
    def get_conversation_stats(self) -> dict:
        ...
        return {
            ...
            "avg_message_length": (
                total_chars / len(self.messages) if self.messages else 0
            ),
        }
    ...
    def test_timeout(self) -> None:
        ...
        result = self.ask_with_timeout(
            "Please take your time responding to this question with a long, detailed answer that will definitely exceed the timeout."
        )
        end_time = time.time()
