...
class GPT_5(AIProvider):
    def __init__(self, api_token: str = ""):
        ...
        if not self.token:
            raise ValueError(
                "GITHUB_GPT_5_TOKEN environment variable not set or api_token not provided"
            )
    ...
    def _call_api(self, message: list[dict[str, str]] | str) -> str:
        ...
        try:
            ...
            if isinstance(message, str):
                ...
                messages.append(
                    SystemMessage(
                        "You are a helpful AI assistant. Always reply briefly, clearly, and to the point."
                    )
                )
                ...
                if not any(
                    isinstance(msg, UserMessage) for msg in messages[1:]
                ):  # Skip system message
...
if __name__ == "__main__":
    ...
    if not token:
        print(
            "⚠️ Please set GITHUB_GPT_5_TOKEN environment variable to test GitHub GPT-5 provider"
        )
        exit(1)
    ...
    try:
        ...
        for i, q in enumerate(questions, 1):
            ...
            print(f"⏳ Response Time {i}: {gpt5.response_time :.2f} seconds")
